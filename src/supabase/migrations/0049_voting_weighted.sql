-- 2026-08-27: Дэвшилтэт зүйл #2 — "Эзэмшлийн хувиар (м²) жинлэсэн
-- санал" (checkbox-оор идэвхжүүлдэг, poll бүрт тусад нь). owners.sqm
-- багана аль хэдийн байгаа тул шинэ багана нэмэхгүйгээр шууд ашиглана.
-- Идэвхжүүлбэл: (1) кворум ба (2) саналын тоолол ХОЁУЛАА толгойн тоогоор
-- бус, sqm-ийн нийлбэрээр тооцогдоно.

alter table voting_polls add column if not exists use_weighted_voting boolean not null default false;
...
begin
  ...
  -- Жинлэсэн горимд саналын тоолол sqm-ийн НИЙЛБЭРЭЭР (толгойн тоо бус)
  -- тооцогдоно. owner_id=null (OwnerApp холбогдоогүй хэрэглэгч) мөрүүд
  -- жинлэсэн горимд 0 жинтэй тооцогдоно (эзэмшлийн хэмжээг мэдэхгүй тул).
      ...
      select jsonb_build_object(
        ...
        'candidates', (
          select coalesce(jsonb_agg(jsonb_build_object(
            'candidate_id', c.id,
            'fullname', c.fullname,
            'votes', case when v_poll.use_weighted_voting then
