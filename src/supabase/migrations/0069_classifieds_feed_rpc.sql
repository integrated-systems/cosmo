-- 2026-08-31: "Зарын самбар"-ийн feed-ийг НЭГ RPC дуудлагаар (post +
-- нэр + реакцын тоо + миний реакц + коммент тоо) татахын тулд.
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'author', coalesce(nullif(trim(both ' ' from concat(o.firstname, ' ', o.lastname)), ''), 'Сууц өмчлөгч'),
...
grant execute on function public.get_classifieds_feed(uuid) to authenticated;

-- Тухайн НЭГ постын комментүүдийг нэртэй нь хамт нэг дуудлагаар.
...
grant execute on function public.get_classifieds_comments(uuid) to authenticated;
